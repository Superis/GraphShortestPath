...
int Node::ShortestPath(int** self_visited, int** opp_visited, int level) {
	...
	for (i = 0; i < endPos; i++) {
		if (opp_visited[neighbor[i]][0] == 1)
			...
			if (self_visited[neighbor[i]][0] != 1) {
...
Buffer::Buffer(int maxVal) : incSize(maxVal), incEnd(0), outSize(maxVal), outEnd(0) {
...
int Buffer::Query(int src, int dest, Index *index) {
	...
	//}
	//for (int i=0;i<l;i++)
	//cout << src_visited[i][0] << " "<< src_visited[i][1] << endl;
		...
		while (1) {
			...
			for (int i = 0; i < l; i++) {
				if (src_visited[i][1] == level - 1) {
					...
					k = this->SearchNodeNeighbours(src_node, 's', src_visited, dest_visited, level);
					if (k > 0) {
						for (int p = 0; p<l; p++) {
							delete[] src_visited[p];
							delete[] dest_visited[p];
			...
			for (int i = 0; i < l; i++) {
				if (dest_visited[i][1] == level - 1) {
					...
					k = this->SearchNodeNeighbours(dest_node, 'd', dest_visited, src_visited, level);
					if (k > 0) {
						for (int p = 0; p<l; p++) {
							delete[] src_visited[p];
							delete[] dest_visited[p];
...
int Buffer::SearchNodeNeighbours(Node* node, char c, int** self, int** opp, int level) {
	int k;
	if (c == 's') {
		do {
